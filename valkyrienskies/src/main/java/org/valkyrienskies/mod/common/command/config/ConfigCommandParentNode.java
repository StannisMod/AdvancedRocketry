...
class ConfigCommandParentNode extends ConfigCommandNode {
    ...
    ConfigCommandParentNode(String name, List<ConfigCommandNode> children) {
        super(name);
        this.children = new HashMap<>(children.stream().collect(Collectors.toMap(c -> c.getName().toLowerCase(), c -> c)));
    }
    ...
    List<ConfigCommandNode> getChildrenStartingWith(String prefix) {
        return children.values().stream().filter(c -> c.getName().toLowerCase().startsWith(prefix.toLowerCase())).collect(Collectors.toList());
    }
