...
| [TASK-47](TASK-47-true-spaceships-valkyrien-skies.md) | True movable spaceships via Valkyrien Skies integration (**feature**, not test-coverage). Block-built ships you can walk on, move, with their own gravity — Path B: integrate VS, don't reimplement physics. Developed on `feature/true_spaceships`. | 🚧 In Progress (Phase 0 GO; Phase 1 done) | Phase 0 cleared (VS via CurseMaven, Apache-2.0, loader coexistence proven dev+obf). Phase 1 soft-dep scaffold landed. Next: Phase 2 (rocket→VS ship), which depends on TASK-48. |
| [TASK-48](TASK-48-freeflight-valkyrien-skies-junction.md) | Free Flight ↔ Valkyrien Skies junction — every assembled rocket is both an EntityRocket and a controllable VS ship. Seam: `IRocketFlightBackend` abstracts who applies the FreeFlightPhysics Step (legacy entity-motion vs VS physics). | 🚧 In Progress (Design; seam proposed) | Cross-branch: needs `feature/true_rcs` (Free Flight) + `feature/true_spaceships` (VS) merged together. 7 open design questions in the task. |
