...
public class ARConfiguration {
    ...
    public static void loadPreInit() {
       ...
       // arConfig.overworldsealevelterraforming = config.get(CLIENT, "overworldSealvlTerraforming", true).getBoolean();
        ...
        arConfig.spaceRegisterUnderTestHarness = config.getBoolean("spaceRegisterUnderTestHarness", PERFORMANCE, false, "Debug/CI only. The space subsystem normally stands down when it detects an automated test harness, because the test probes register their own dimension pool and two pools would fight over slot ids. Setting this true makes it register anyway, so an automated test can exercise the real server-start and world-save path. Leave false: on a normal server this flag does nothing at all, since no harness is ever detected.");
