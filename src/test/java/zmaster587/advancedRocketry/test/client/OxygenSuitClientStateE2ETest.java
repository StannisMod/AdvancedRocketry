...
public class OxygenSuitClientStateE2ETest extends AbstractClientE2ETest {
    ...
    public void vacuumDamageReachesTheClient() throws Exception {
        ...
        try {
            ...
            // Stand the player on surveyed natural ground (see HarnessPlayerSite). This test
            // asserts only that health DROPS, so any other damage source — suffocation in a
            // hillside, a fall, drowning after sliding off a one-block pillar into the sea —
            // would satisfy it for the wrong reason. The site is what keeps the green honest.
            serverClient().execute(HarnessPlayerSite.tpCommand());
            bot().waitTicks(10);
